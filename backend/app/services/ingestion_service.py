...
class IngestionService:
    ...
    def __init__(
        ...
        self._skill_matching_service = SkillMatchingService()
    ...
    async def ingest(self, source: DataSource) -> IngestionRun:
        ...
        try:
            ...
            for job in jobs:
                ...
                posting = self._upsert_job_posting(
                    source.id,
                    normalized,
                    retrieved_at,
                )
            ...
            self._skill_matching_service.match_job_postings(
                self._session,
                posting_ids,
            )
    ...
    def _normalize_muse_job(self, job: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "source_url": self._required_nested_string(
                job,
                "refs",
                "landing_page",
            ),
            "title": self._required_string(job, "name"),
            "company_name": self._optional_nested_string(
                job,
                "company",
                "name",
            ),
            "published_at": self._required_datetime(
                job,
                "publication_date",
            ),
            ...
            "location_scope": None,
            "is_remote": any(
                "remote" in location.casefold() for location in locations
            ),
    ...
    def _normalize_remotive_job(
        ...
        return {
            ...
            "company_name": self._optional_string(
                job,
                "company_name",
            ),
            "published_at": self._required_datetime(
            ...
            "location_raw": self._optional_string(
                job,
                "candidate_required_location",
            ),
            "location_scope": self._optional_string(
                job,
                "candidate_required_location",
            ),
            "is_remote": True,
    ...
    def _upsert_job_posting(
        ...
        """Create or update one canonical job posting"""
    ...
    def _optional_nested_string(
        ...
        return (
            self._optional_string(value, key)
